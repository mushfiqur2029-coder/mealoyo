          ...
          {filtered.length === 0 ? (
            ...
            <div key={d.id} className="urow" style={{display:'flex', alignItems:'center', gap:14, padding:'15px 22px', borderBottom:i < filtered.length - 1 ? '1px solid var(--border-subtle)' : 'none', transition:'background 0.12s'}}>
              ...
              <div className="action-row" style={{display:'flex', gap:8, flexShrink:0, flexWrap:'wrap'}}>
                <button className="view-btn action-btn" onClick={() => setProfileTarget(d)} style={{height:34, padding:'0 14px', background:'transparent', color:'var(--text-primary)', border:'1px solid var(--border-subtle)', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'all 0.12s'}}>Profile</button>
                ...
                {d.status !== 'suspended' && (
                  <button className="reject action-btn" disabled={busyId === d.id} onClick={() => setSuspendTarget(d)} style={{height:34, padding:'0 14px', background:'#F59E0B', color:'#fff', border:'none', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'background 0.12s', opacity:busyId === d.id ? 0.6 : 1}}>Suspend</button>
                )}
                <button className="del-row action-btn" onClick={() => setDeleteTarget(d)} style={{height:34, padding:'0 14px', background:'transparent', color:'#DC2626', border:'1px solid rgba(220,38,38,0.55)', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'all 0.12s'}}>Delete</button>
              </div>
            </div>
          ))}
        </div>
