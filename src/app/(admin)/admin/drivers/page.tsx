    ...
    <div style={{minHeight:'100vh',background:'#0D0006',fontFamily:'Inter,system-ui,sans-serif'}}>
        ...
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 20px',height:62,display:'flex',alignItems:'center'}}>
          ...
          <div style={{display:'flex',gap:0,flex:1}}>
            {[{l:'Dashboard',h:'/admin/dashboard'},{l:'Sellers',h:'/admin/sellers'},{l:'Drivers',h:'/admin/drivers'},{l:'Orders',h:'/admin/orders'},{l:'Settings',h:'/admin/settings'}].map(t=>(
              <Link key={t.h} href={t.h} className="nav-link" style={{height:62,padding:'0 12px',display:'flex',alignItems:'center',fontSize:13,fontWeight:t.h==='/admin/drivers'?700:400,color:t.h==='/admin/drivers'?'#C8006A':'rgba(255,255,255,0.5)',borderBottom:t.h==='/admin/drivers'?'2.5px solid #C8006A':'2.5px solid transparent',transition:'color 0.12s'}}>{t.l}</Link>
            ))}
          </div>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 20px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:24,flexWrap:'wrap',gap:12}}>
          <div>
            <h1 style={{fontFamily:'Georgia,serif',fontSize:'clamp(22px,2.5vw,28px)',fontWeight:700,color:'#fff',marginBottom:4}}>Drivers</h1>
            <p style={{fontSize:14,color:'rgba(255,255,255,0.45)'}}>{drivers.length} {drivers.length===1?'driver':'drivers'} registered.</p>
          </div>
            ...
            <div key={d.id} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 22px',borderBottom:i<filtered.length-1?'1px solid rgba(255,255,255,0.05)':'none'}}>
              ...
              <div style={{display:'flex',gap:18,flexShrink:0,marginRight:8}}>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:14,fontWeight:700,color:'#fff'}}>{deliveryCounts[d.id]||0}</div>
                  <div style={{fontSize:9,color:'rgba(255,255,255,0.35)',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.04em'}}>Deliveries</div>
                </div>
              </div>
              <div style={{display:'flex',gap:8,flexShrink:0}}>
                {d.status !== 'active' && (
                  <button className="approve" disabled={busyId===d.id} onClick={()=>setStatus(d.id,'active')} style={{height:32,padding:'0 16px',background:'#2DA84E',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',transition:'background 0.12s',opacity:busyId===d.id?0.6:1}}>{d.status==='pending'?'Approve':'Reactivate'}</button>
                )}
                {d.status !== 'suspended' && (
                  <button className="reject" disabled={busyId===d.id} onClick={()=>setStatus(d.id,'suspended')} style={{height:32,padding:'0 14px',background:'rgba(192,57,43,0.8)',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',transition:'background 0.12s',opacity:busyId===d.id?0.6:1}}>Suspend</button>
                )}
              </div>
            </div>
          ))}
