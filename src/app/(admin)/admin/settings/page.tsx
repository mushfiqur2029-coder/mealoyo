    ...
    <div style={{minHeight:'100vh', background:'var(--bg-page)', fontFamily:'Inter,system-ui,sans-serif'}}>
      ...
      <div style={{maxWidth:1200, margin:'0 auto', padding:'32px 20px 56px'}}>
        <div className="fade-up" style={{marginBottom:26}}>
          <h1 style={{fontFamily:'Georgia,serif', fontSize:'clamp(22px,2.5vw,30px)', fontWeight:700, color:'var(--text-primary)', letterSpacing:'-0.02em', marginBottom:4}}>Settings</h1>
          <p style={{fontSize:14, color:'var(--text-secondary)'}}>Platform configuration and stats.</p>
        </div>

        <h2 className="fade-up" style={{fontFamily:'Georgia,serif', fontSize:17, fontWeight:700, color:'var(--text-primary)', marginBottom:14}}>Platform stats</h2>
        <div className="stats-grid fade-up" style={{display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:14, marginBottom:32}}>
          {statCards.map((s, i) => (
            <div key={i} className="stat-card" style={{background:'var(--bg-card)', borderRadius:16, padding:'18px', border:'1px solid var(--border-subtle)', textAlign:'center'}}>
              ...
              <div style={{fontSize:11, color:'var(--text-secondary)', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.05em'}}>{s.l}</div>
            </div>
          ))}
        </div>

        <div className="config-grid fade-up" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:20}}>
          <div style={{background:'var(--bg-card)', borderRadius:18, padding:'24px', border:'1px solid var(--border-subtle)'}}>
            <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:6}}>
              ...
              <h2 style={{fontFamily:'Georgia,serif', fontSize:16, fontWeight:700, color:'var(--text-primary)'}}>Commission rate</h2>
            </div>
            <p style={{fontSize:13, color:'var(--text-secondary)', marginBottom:16, lineHeight:1.6}}>Platform commission taken from every delivered order. Set in code, not yet editable here.</p>
          ...
          <div style={{background:'var(--bg-card)', borderRadius:18, padding:'24px', border:'1px solid var(--border-subtle)'}}>
            <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:6}}>
              ...
              <h2 style={{fontFamily:'Georgia,serif', fontSize:16, fontWeight:700, color:'var(--text-primary)'}}>Add an admin</h2>
            </div>
            <p style={{fontSize:13, color:'var(--text-secondary)', marginBottom:16, lineHeight:1.6}}>Promotes an existing registered meaLoyo user to admin by email. They must already have an account.</p>
            ...
            <form onSubmit={handlePromote} style={{display:'flex', gap:8}}>
              <input type="email" value={promoteEmail} onChange={e => setPromoteEmail(e.target.value)} placeholder="user@example.com" required style={{flex:1, height:44, border:'1px solid var(--border-subtle)', borderRadius:10, padding:'0 14px', fontSize:13, color:'var(--text-primary)', background:'var(--bg-card)', outline:'none', transition:'border-color 0.14s'}}/>
        ...
        {/* Appearance */}
        <div className="fade-up" style={{marginTop:24, background:'var(--bg-card)', borderRadius:18, padding:'22px 24px', border:'1px solid var(--border-subtle)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap'}}>
          <div style={{display:'flex', alignItems:'center', gap:10}}>
            <div style={{width:38, height:38, borderRadius:10, background:'rgba(200,0,106,0.16)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18}}>🎨</div>
            <div>
              <h2 style={{fontFamily:'Georgia,serif', fontSize:16, fontWeight:700, color:'var(--text-primary)'}}>Appearance</h2>
              <p style={{fontSize:12.5, color:'var(--text-secondary)'}}>Light, dark, or match your device.</p>
            </div>
          </div>
          <ThemeToggle/>
        </div>
        ...
        <div className="fade-up" style={{marginTop:32}}>
          <h2 style={{fontFamily:'Georgia,serif', fontSize:17, fontWeight:700, color:'var(--text-primary)', marginBottom:6}}>Deletion history</h2>
          <p style={{fontSize:13, color:'var(--text-secondary)', marginBottom:14}}>Recent deletions across the platform — who removed what, and when.</p>
              ...
              <div key={d.id} style={{display:'flex', alignItems:'center', gap:14, padding:'14px 22px', borderBottom:i < deletions.length - 1 ? '1px solid var(--border-subtle)' : 'none'}}>
                ...
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:14, fontWeight:700, color:'var(--text-primary)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    ...
                    <span style={{fontSize:10, fontWeight:700, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.04em', marginLeft:8, background:'var(--bg-secondary)', padding:'2px 8px', borderRadius:20}}>{d.entity_type}</span>
                  </div>
                  <div style={{fontSize:12, color:'var(--text-secondary)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    by {d.deleted_by ? (deleterNames[d.deleted_by] || `${d.deleted_by.slice(0, 8)}…`) : 'Unknown'}
                  </div>
                </div>
                <div style={{fontSize:12, color:'var(--text-secondary)', flexShrink:0, textAlign:'right', whiteSpace:'nowrap'}}>
