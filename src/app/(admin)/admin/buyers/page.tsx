          ...
          {filtered.length === 0 ? (
            ...
            <div key={b.id} className="urow" style={{display:'flex', alignItems:'center', gap:14, padding:'15px 22px', borderBottom:i < filtered.length - 1 ? '1px solid var(--border-subtle)' : 'none', transition:'background 0.12s'}}>
              ...
              <div className="action-row" style={{display:'flex', gap:8, flexShrink:0, flexWrap:'wrap'}}>
                ...
                {b.status !== 'suspended' && (
                  <button className="reject action-btn" disabled={busyId === b.id} onClick={() => setStatus(b.id, 'suspended')} style={{height:34, padding:'0 14px', background:'#F59E0B', color:'#fff', border:'none', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'background 0.12s', opacity:busyId === b.id ? 0.6 : 1}}>Suspend</button>
                )}
