...
const dark = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-font-smoothing: antialiased; }
  ...
  @media (max-width: 640px) { .ucounts { display: none !important; } .search { width: 100% !important; } .action-row { flex-direction: column !important; align-items: stretch !important; width: 100% !important; } .action-row > button { width: 100% !important; } }
`
...
export default function AdminBuyers() {
  ...
  const confirmSuspend = async () => {
    ...
    setBuyers(prev => prev.map(b => b.id === suspendTarget.id ? { ...b, status: 'suspended' } : b))
    setSuspending(false)
          ...
          {filtered.length === 0 ? (
            ...
            <div key={b.id} className="urow" style={{display:'flex', alignItems:'center', gap:14, padding:'15px 22px', borderBottom:i < filtered.length - 1 ? '1px solid var(--border-subtle)' : 'none', transition:'background 0.12s'}}>
              ...
              <div className="action-row" style={{display:'flex', gap:8, flexShrink:0, flexWrap:'wrap'}}>
                ...
                {b.status !== 'suspended' && (
                  <button className="reject action-btn" disabled={busyId === b.id} onClick={() => setSuspendTarget(b)} style={{height:34, padding:'0 14px', background:'#F59E0B', color:'#fff', border:'none', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'background 0.12s', opacity:busyId === b.id ? 0.6 : 1}}>Suspend</button>
                )}
                <button className="del-row action-btn" onClick={() => setDeleteTarget(b)} style={{height:34, padding:'0 14px', background:'transparent', color:'#DC2626', border:'1px solid rgba(220,38,38,0.55)', borderRadius:8, fontSize:12.5, fontWeight:700, cursor:'pointer', transition:'all 0.12s'}}>Delete</button>
              </div>
            </div>
          ))}
        </div>
