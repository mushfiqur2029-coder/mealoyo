...
export default function About() {
  return (
    <div style={{minHeight:'100vh',background:'var(--bg-page)',fontFamily:'Inter,system-ui,sans-serif'}}>
      ...
      <nav style={{background:'var(--bg-nav)',borderBottom:'1px solid var(--border-subtle)',position:'sticky',top:0,zIndex:100,height:62}}>
        <div style={{maxWidth:820,margin:'0 auto',padding:'0 20px',height:62,display:'flex',alignItems:'center',gap:14}}>
          <Link href="/" style={{width:34,height:34,border:'1.5px solid var(--border-subtle)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,color:'var(--text-primary)',flexShrink:0}}>←</Link>
      ...
      <div style={{maxWidth:760,margin:'0 auto',padding:'52px 20px 72px'}}>
        ...
        <div style={{marginBottom:36}}>
          <h2 style={{fontFamily:'Georgia,serif',fontSize:24,fontWeight:700,color:'var(--text-primary)',marginBottom:12,letterSpacing:'-0.01em'}}>Why we started meaLoyo</h2>
          <p style={{fontSize:15,color:'var(--text-primary)',lineHeight:1.85,marginBottom:14}}>Some of the best food in Britain never makes it onto a menu. It is cooked at home — Bangladeshi, Pakistani, Indian, Caribbean, Middle Eastern, West African and more — by talented cooks who have never had a simple, trusted way to share it with their neighbours.</p>
          <p style={{fontSize:15,color:'var(--text-primary)',lineHeight:1.85}}>meaLoyo gives those cooks a storefront, handles payments and delivery, and gives buyers a safe, verified way to order the real thing. Every cook is ID checked and food-hygiene certified before they can sell.</p>
        </div>

        {/* Mission card — brand-pink surface stays light in both themes, so its text stays dark. */}
        ...
        {/* 12% commission USP */}
        <div style={{marginBottom:36}}>
          <h2 style={{fontFamily:'Georgia,serif',fontSize:24,fontWeight:700,color:'var(--text-primary)',marginBottom:16,letterSpacing:'-0.01em'}}>The fairest deal for cooks</h2>
          <div style={{display:'flex',alignItems:'center',gap:20,background:'var(--bg-card)',border:'1.5px solid var(--border-subtle)',borderRadius:18,padding:'24px 26px',boxShadow:'0 4px 18px var(--shadow-card)',flexWrap:'wrap'}}>
            <div style={{flexShrink:0}}>
              <div style={{fontFamily:'Georgia,serif',fontSize:54,fontWeight:700,color:'#C8006A',lineHeight:1}}>12%</div>
              <div style={{fontSize:11,fontWeight:700,color:'var(--text-primary)',textTransform:'uppercase',letterSpacing:'0.06em',marginTop:4}}>flat commission</div>
            </div>
            <p style={{fontSize:15,color:'var(--text-primary)',lineHeight:1.8,flex:1,minWidth:220}}>While the big delivery apps take up to 30% from every order, meaLoyo charges a flat <strong>12% commission</strong>. Cooks keep <strong>88%</strong> of what they sell — so more of your money goes to the person who actually cooked your meal.</p>
          </div>
        </div>
        ...
        <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
          ...
          <Link href="/" style={{height:50,padding:'0 28px',display:'flex',alignItems:'center',border:'1.5px solid var(--border-subtle)',borderRadius:12,fontSize:15,fontWeight:600,color:'var(--text-primary)'}}>Browse food →</Link>
        </div>
