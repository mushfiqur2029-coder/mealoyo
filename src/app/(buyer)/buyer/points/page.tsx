  ...
  const pageStyles = (
    <style>{`
      ...
      * { scrollbar-width: none; -ms-overflow-style: none; }
      ...
      .hrow:hover { background: #FFF5FA !important; }
    ...
    <div style={{minHeight:'100vh', background:'#F8F0F4', fontFamily:'Inter,system-ui,sans-serif'}}>
      ...
      <div style={{maxWidth:760, margin:'0 auto', padding:'32px 20px 56px'}}>
        ...
        <div className="fade-up" style={{marginBottom:22}}>
          <h1 style={{fontFamily:'Georgia,serif', fontSize:'clamp(24px,3vw,34px)', fontWeight:700, color:'#1A1A1A', letterSpacing:'-0.02em', marginBottom:4}}>Loyalty points</h1>
          <p style={{fontSize:14, color:'#1A1A1A', opacity:0.85}}>Earn {POINTS_PER_POUND_EARN} points for every £1 you spend. {POINTS_PER_POUND_REDEEM} points = £1 off.</p>
        </div>
        ...
        <div className="fade-up" style={{background:'linear-gradient(135deg,#C8006A 0%,#8B0047 100%)', borderRadius:22, padding:'30px 28px', boxShadow:'0 10px 30px rgba(200,0,106,0.28)', position:'relative', overflow:'hidden', marginBottom:20}}>
          ...
          <Link href="/" className="browse-btn" style={{display:'inline-flex', alignItems:'center', height:46, padding:'0 22px', marginTop:20, background:'#fff', color:'#C8006A', borderRadius:11, fontSize:14, fontWeight:700, transition:'all 0.16s'}}>Browse food to earn more →</Link>
        </div>
        ...
        <div className="pts-stats fade-up" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:20}}>
          <div style={{background:'#fff', borderRadius:18, padding:'20px', boxShadow:'0 2px 14px rgba(200,0,106,0.06)', border:'1.5px solid rgba(200,0,106,0.07)'}}>
        ...
        <div className="fade-up" style={{background:'#fff', borderRadius:20, overflow:'hidden', boxShadow:'0 2px 16px rgba(200,0,106,0.07)', border:'1.5px solid rgba(200,0,106,0.07)'}}>
          <div style={{padding:'18px 22px', borderBottom:'1px solid #F5F0F3'}}>
            <h3 style={{fontFamily:'Georgia,serif', fontSize:17, fontWeight:700, color:'#1A1A1A'}}>Points history</h3>
          </div>
          {history.length === 0 ? (
            <div style={{padding:'48px 32px', textAlign:'center'}}>
              <div style={{fontSize:40, marginBottom:12}}>⭐</div>
              <p style={{fontSize:14, color:'#1A1A1A', marginBottom:18, lineHeight:1.6, maxWidth:340, margin:'0 auto 18px'}}>No points yet. Place an order and you’ll earn {POINTS_PER_POUND_EARN} points for every £1 once it’s delivered.</p>
            ...
            return (
              <div key={h.id} className="hrow" style={{display:'flex', alignItems:'center', gap:13, padding:'15px 22px', borderBottom:i < history.length - 1 ? '1px solid #F5F0F3' : 'none', transition:'background 0.12s'}}>
                ...
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:14, fontWeight:700, color:'#1A1A1A'}}>{earned ? 'Points earned' : 'Points redeemed'}</div>
                  <div style={{fontSize:12, color:'#1A1A1A', opacity:0.8, marginTop:1}}>{h.description || (earned ? 'Order reward' : 'Discount applied')} · {fmtDate(h.created_at)}</div>
                </div>
