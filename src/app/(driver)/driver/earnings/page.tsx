    ...
    <div style={{minHeight:'100vh', background:'#0D0D0D', fontFamily:'Inter,system-ui,sans-serif'}}>
        ...
        <div className="bottom-grid" style={{display:'grid', gridTemplateColumns:'minmax(0,1.7fr) minmax(0,320px)', gap:20, alignItems:'start'}}>
          ...
          <div className="fade-up" style={{position:'sticky', top:84, display:'flex', flexDirection:'column', gap:16}}>
            <div style={{background:'rgba(255,255,255,0.05)', borderRadius:18, border:'1px solid rgba(255,255,255,0.08)', padding:'22px'}}>
              ...
              <button onClick={openModal} disabled={!canWithdraw} className="prim" style={{width:'100%', height:46, background:canWithdraw ? '#C8006A' : 'rgba(255,255,255,0.1)', color:canWithdraw ? '#fff' : 'rgba(255,255,255,0.4)', border:'none', borderRadius:12, fontSize:14, fontWeight:700, cursor:canWithdraw ? 'pointer' : 'not-allowed', marginBottom:12, transition:'background 0.14s'}}>Withdraw funds</button>
            ...
            <div style={{background:'rgba(255,255,255,0.03)', borderRadius:18, border:'1px solid rgba(255,255,255,0.07)', padding:'18px 20px'}}>
              ...
              {withdrawals.length === 0 ? (
                ...
                return (
                  <div key={w.id} style={{padding:'10px 0', borderTop:'1px solid rgba(255,255,255,0.05)'}}>
                    ...
                    {w.status === 'rejected' && (w.rejection_reason || w.admin_note) && <div style={{fontSize:11, color:'#FF8A8A', marginTop:4}}>Reason: {w.rejection_reason || w.admin_note}</div>}
                    {w.status === 'paid' && (
                      <div style={{marginTop:6, background:'rgba(45,168,78,0.14)', border:'1px solid rgba(52,211,153,0.3)', borderRadius:8, padding:'8px 10px'}}>
                        <div style={{fontSize:11.5, color:'#34D399', fontWeight:700}}>✅ Payment confirmed — check your bank account</div>
                        {w.receipt_url && <a href={w.receipt_url} target="_blank" rel="noopener noreferrer" style={{fontSize:11.5, color:'#34D399', fontWeight:600, textDecoration:'underline', display:'inline-block', marginTop:3}}>Download receipt →</a>}
                      </div>
                    )}
                  </div>
                )
      ...
      {modalOpen && (
        <div onClick={closeModal} style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(4px)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20}}>
          <div onClick={e => e.stopPropagation()} className="fade-up" style={{background:'#161616', borderRadius:20, width:'100%', maxWidth:440, maxHeight:'90vh', overflowY:'auto', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 20px 60px rgba(0,0,0,0.6)'}}>
            {submitted ? (
              <div style={{padding:'40px 32px', textAlign:'center'}}>
                ...
                <h2 style={{fontFamily:'Georgia,serif', fontSize:22, fontWeight:700, color:'#fff', marginBottom:10}}>Withdrawal request submitted</h2>
                <p style={{fontSize:14, color:'rgba(255,255,255,0.6)', lineHeight:1.6, marginBottom:24}}>Admin will review and process within 2–3 business days. You&apos;ll see the status update here once it&apos;s paid.</p>
              ...
              <div style={{padding:'26px 28px 28px'}}>
                <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:18}}>
                  <h2 style={{fontFamily:'Georgia,serif', fontSize:21, fontWeight:700, color:'#fff'}}>Withdraw funds</h2>
                  <button onClick={closeModal} aria-label="Close" style={{width:32, height:32, borderRadius:9, border:'1px solid rgba(255,255,255,0.15)', background:'transparent', fontSize:15, color:'rgba(255,255,255,0.7)', cursor:'pointer'}}>✕</button>
                </div>
                ...
                <div style={{background:'rgba(255,255,255,0.05)', borderRadius:14, padding:'16px 18px', marginBottom:18, border:'1px solid rgba(255,255,255,0.07)'}}>
                  <div style={{fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.5)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:4}}>Amount to withdraw</div>
                ...
                {!bankSaved ? (
                  ...
                  <>
                    <div style={{fontSize:12, fontWeight:700, color:'#fff', marginBottom:10}}>Payment goes to</div>
                    {[
                      ...
                      <div key={f.l} style={{marginBottom:12}}>
                        <label style={{display:'block', fontSize:11, fontWeight:600, color:'rgba(255,255,255,0.5)', marginBottom:5}}>{f.l}</label>
                        <input value={f.v || ''} readOnly style={{width:'100%', height:42, border:'1px solid rgba(255,255,255,0.12)', borderRadius:10, padding:'0 14px', fontSize:14, fontWeight:600, color:'#fff', background:'rgba(255,255,255,0.04)', cursor:'not-allowed'}}/>
                      </div>
                    ))}
                    <p style={{fontSize:11.5, color:'rgba(255,255,255,0.5)', lineHeight:1.55, marginTop:6, marginBottom:18}}>Payments are only made to the account registered under your name. For security, bank details can only be changed in your <Link href="/driver/profile" style={{color:'#FF8AC4', fontWeight:600}}>Profile settings</Link>.</p>
                  </>
                )}
                ...
                <button onClick={() => handleRequestWithdrawal(available)} disabled={!bankSaved || requesting || available < 0.01} className="prim" style={{width:'100%', height:48, background:(!bankSaved || requesting || available < 0.01) ? 'rgba(255,255,255,0.1)' : '#C8006A', color:(!bankSaved || requesting || available < 0.01) ? 'rgba(255,255,255,0.4)' : '#fff', border:'none', borderRadius:12, fontSize:15, fontWeight:700, cursor:(!bankSaved || requesting || available < 0.01) ? 'not-allowed' : 'pointer'}}>{requesting ? 'Submitting…' : 'Request withdrawal'}</button>
              </div>
            )}
          </div>
        </div>
