...
// ── DISH DETAIL PAGE ─────────────────────────────────
fs.mkdirSync('src/app/dish/[id]', { recursive: true })
fs.writeFileSync('src/app/dish/[id]/page.tsx', `
'use client'
...
`)
console.log('Dish detail page done')

// ── ORDER CONFIRMATION PAGE ──────────────────────────
fs.mkdirSync('src/app/(buyer)/buyer/orders/[id]', { recursive: true })
fs.writeFileSync('src/app/(buyer)/buyer/orders/[id]/page.tsx', `
'use client'
import { useState, useEffect } from 'react'
import Link from 'next/link'
import { supabase } from '@/lib/supabase'
...
`)
console.log('Order page done')

// ── BUYER ORDERS LIST ────────────────────────────────
fs.mkdirSync('src/app/(buyer)/buyer/orders', { recursive: true })
fs.writeFileSync('src/app/(buyer)/buyer/orders/page.tsx', `
'use client'
import { useState, useEffect } from 'react'
import Link from 'next/link'
import { supabase } from '@/lib/supabase'
...
`)
console.log('Buyer orders list done')

// ── UPDATE HOMEPAGE — link Order button to dish page ─
console.log('Updating homepage order buttons...')
let homePage = fs.readFileSync('src/app/page.tsx', 'utf8')
homePage = homePage.replace(
  `<Link href={\`/register\`} className="order-btn"`,
  `<Link href={\`/dish/\${l.id}\`} className="order-btn"`
)
fs.writeFileSync('src/app/page.tsx', homePage)
console.log('Homepage order buttons updated')

console.log('')
console.log('════════════════════════════════')
console.log('PHASE 6 DONE')
console.log('════════════════════════════════')
console.log('/dish/[id]              — dish detail + order form')
console.log('/buyer/orders           — all orders list')
console.log('/buyer/orders/[id]      — order tracking + review')
console.log('')
console.log('Full order flow:')
console.log('Homepage → Click dish → Order form → Confirmation → Track → Review')